...
### CORE-EXECUTOR-SPLIT — Core + executor packages — plan non-engineering work without a mode flag · 🟢 Active · ~18 sessions remaining
...
> Exec plan: S4→S5→S6→S7→S8→S9→S10
...
| S4 | `/slice gauntlet-split` | 🟡 Next | ~4 | S3 | Protocol, decisions, authorization, receipts, evaluation packets into core; githubClient becomes the github-pr adapter in exec-engineering; break the gauntlet/portfolio/evaluate/dispatch cycles; contract test. |
...
- **Status:** 🟡 Next (CORE-EXECUTOR-SPLIT · S4)
- **Deps:** S3
- **Read-order:**
