...
- **[P0]** `/slice estimate-native` — Port shapes/risks/PERT/calibration cascade/JSONL history to JS with agent-time's tests; keep meta.estimation.engine as an optional override. · touches `estimate-core.mjs`, `estimate.mjs`, `run.mjs`
...
### CORE-EXECUTOR-SPLIT — Core + executor packages — plan non-engineering work without a mode flag · 🟢 Active · ~25 sessions remaining
...
> Exec plan: S1→S2→S3→S4→S5→S6→S7→S8→S9→S10
...
|---|---|---|---|---|---|
| S1 | `/slice estimate-native` | 🟡 Next · **P0** | ~3 | — | Port shapes/risks/PERT/calibration cascade/JSONL history to JS with agent-time's tests; keep meta.estimation.engine as an optional override. |
| S2 | `/slice workspaces` | ⚪ Scheduled | ~1 | S1 | Create the four workspace packages with package.json + READMEs; root bin still works; decide the tarball `files` deliberately. |
...
### `estimate-native`
- **What:** Port shapes/risks/PERT/calibration cascade/JSONL history to JS with agent-time's tests; keep meta.estimation.engine as an optional override.
- **Status:** 🟡 Next (CORE-EXECUTOR-SPLIT · S1)
- **Priority:** P0 · weight 90 — core must price natively before estimate-core moves into packages/core
- **Read-order:**
  1. docs/ARCHITECTURE.md (target layout; estimate-core is core)
  2. scripts/lib/estimate-core.mjs
  3. agent-time skills/agent-time-estimator/references/model.md
- **Resume / next action:** Port estimator.py's CONFIG/SHAPES/RISKS tables and compute_estimate into estimate-core.mjs; runEstimate uses it unless meta.estimation.engine is set.
...
- **Status:** ⚪ Scheduled (CORE-EXECUTOR-SPLIT · S2)
- **Deps:** S1
- **Read-order:**
