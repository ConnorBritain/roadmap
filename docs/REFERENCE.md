# Reference — the engineering profile in depth

This is the long-form reference that used to be the README: the three surfaces (CLI, plugin, MCP),
the `roadmap.yaml` model, the backlog, the Gauntlet, Linear, cloud dispatch, estimation, review
discipline, the disk ceiling, and what's built. It describes the **engineering** work profile
(`meta.profile` absent or `engineering`). The general profile's surfaces are in
[`packages/exec-general/README.md`](../packages/exec-general/README.md); the split itself is in
[`ARCHITECTURE.md`](ARCHITECTURE.md). Start from the [README](../README.md).
...
## Estimation & timeline (optional) — the agent-time model, built in

`target_date` is the one roadmap date that's usually a guess. The roadmap ships the [**agent-time**](https://github.com/ConnorBritain/agent-time) estimation model natively (`packages/core/src/estimator-core.mjs`, a port of its `estimator.py`): it prices a task in calibrated *agent-rounds → wall-clock minutes* (never human hours) and self-corrects from logged outcomes. No Python is needed. The calibration history is agent-time's own `history.jsonl` format at the same path (`<repo>/.claude/agent-time/history.jsonl`, or `$AGENT_TIME_DATA`), so an existing history keeps calibrating and agent-time's round-counter hook keeps auto-filling actuals. The roadmap turns those durations into a **projected timeline**:
...
- **`roadmap estimate log <slice> --status pass|fail|partial|abandoned`** closes the **calibration loop** — it records the slice's outcome to the history so the next estimate self-corrects. Idempotent per estimate (safe to re-run). The journal Stop-hook fires it automatically when a mapped slice finishes. Actual rounds/minutes auto-fill **only if agent-time's round-counter (`PostToolUse`) hook ran in that session** — otherwise pass `--actual-rounds N` (an outcome with no actuals is rejected, never guessed). Enable that hook per-repo from agent-time's `hooks/README.md` to make auto-calibration real.
...
Config lives in `meta.estimation` (`hours_per_day` throughput, `expected|high` point, `model` for the per-model horizon). To keep using an external `estimator.py` instead of the built-in model, set `meta.estimation.engine` (or `$AGENT_TIME_ENGINE`) to its path — it is used only when explicitly configured, and a configured path that is missing is an error, never a silent fallback. Either way the same calibrated numbers that size one task plan the whole program.
...
- **Graph brain** (`packages/core/src/graph.mjs`): dependency resolution (sibling / fully-qualified / whole-PI deps), cycle detection, wave scheduling with two-wave file-contention + priority-first cap packing, sessions-remaining and exec-plan derivation.
- **Priority** (`packages/core/src/priority.mjs`): the shared tier/weight/reason model — validation, the derived comparator, tier badges.
- **Backlog** (`packages/core/src/backlog-core.mjs` + `scripts/backlog.mjs`, `grab.mjs`, `promote.mjs`, `next.mjs`): the erratic-work tracker — capture/triage/launch/promote, `BACKLOG.md` rendering, the item→node adapter that reuses the fanout brief machinery, and cross-tracker pick-next.
- **Renderer** (`packages/core/src/render-core.mjs`): hierarchical `SLICES.md` (per-PI sections, nested sprint tables, priority badges, derived exec-plan lines, cross-PI wave map, held-on-human, backlog pointer) + `BACKLOG.md`.
- **Scheduler** (`scripts/scheduler.mjs` over `packages/core/src/plan.mjs`; capacity and per-node annotation come from the loaded profile's Executor): recommended-cap eval (CPU / RAM / independent-work / review / **disk** ceilings, reporting which binds) plus the waves.
- **Fanout** (`packages/exec-engineering/src/worktree-session.mjs`, the `worktree-session` Executor; `scripts/fanout.mjs` is its CLI): `tmux`, `wt`, `warp`, `print`, `background` adapters; per-slice worktree plus synthesized kickoff brief (stashed `prompt` embedded, leftover-capture instruction); disk hard-block; the interactive console (`wizard.mjs`); guarded launch (`--dry`/`--out` preview, autonomous double-ack); `cleanup.mjs` worktree pruning.
- **Shared mutation store** (`packages/core/src/store.mjs`): the one read → mutate → validate → write → re-render path under every mutating surface (MCP, `set`, `backlog`, `promote`), including the validate-both-before-writing-either two-file promote.
...
- **MCP server** (`scripts/mcp.mjs`, server `graph`; core tools plus the loaded profile's): bundled JSON-RPC stdio tool families for graph reads,
  ...
  writing.
- **PR-watch monitor** (`scripts/watch-prs.mjs` + `packages/exec-engineering/src/pr-watch-core.mjs` + `monitors/monitors.json`):
