...
3. **Check execution state — by profile.** Engineering (`meta.profile` absent or `engineering`): `git rev-parse --abbrev-ref HEAD` + `git fetch origin --quiet` + `git status -s`; compare against the slice's expected branch; note drift or dirty files. General (`meta.profile: general`): check the slice's `artifact` path — does the file exist, when was it last committed (`git log -1 --format=%h%x20%cs -- <path>`), and is there an assignment brief under `.roadmap/assignments/<key>*.md` (who, since when)?
4. **Print a short orientation** (Slice/Status · What · Priority if set · Read-order: N read · Branch: sync/drift/dirty — or, general: Artifact + assignee · Next · Gate — a checklist gate is listed line by line). If the slice carries a `Prompt:` block (author-stashed instructions), relay it **verbatim** — it is the pickup brief. Then **STOP** and wait.
