---
name: slice-init
description: Bootstrap a repo's roadmap graph (docs/roadmap/roadmap.yaml) when none exists — a PM-style interview that warm-starts from any existing roadmap docs, then writes the YAML and renders SLICES.md. Use to onboard a new repo to slice-roadmap.
argument-hint: "[--cold]"
...
1. **If a roadmap already exists**, say so and suggest `/slice-sync` instead. Stop unless the user wants to re-bootstrap.
