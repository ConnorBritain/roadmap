...
6. **Cross-harness.** `--harness claude|codex|generic` (or `meta.harness`) selects the agent CLI: `codex` launches `codex exec` and rewrites the directive into Codex's dialect; the directive degrades (never errors) when a mode isn't native. For an ACP orchestrator (OpenClaw/Hermes/editor), `roadmap fan --emit acp` writes a wave manifest of `sessions_spawn`-shaped specs instead of launching — `--agent <id>` names the target. See `docs/cross-harness.md`.
