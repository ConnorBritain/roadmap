...
import {
  loadGraph, flatten, computeWaves, execPlan, sessionsRemaining,
  statusDisplay, emojiFor, isDone, STATUS,
} from "./lib/graph.mjs";
...
const cap = Number(flag("--cap", (graph.meta && graph.meta.default_concurrency) || 3));
...
// ---- header / intro -------------------------------------------------------
w("<!-- GENERATED from docs/roadmap/roadmap.yaml by /slice-sync (render.mjs). Do not edit this file directly — edit the YAML and re-render. -->");
...
w("");
w("Use `/slice <name>` to orient on a menu entry; `/slice-fanout` reads the same graph to launch concurrent sessions; `/slice-sync` re-renders this file from the YAML after merges. `/pickup` resumes a specific in-progress session.");
w("");

// ---- cross-PI wave map ----------------------------------------------------
w(`## Ready now — wave map (cap ${cap})`);
w("");
w("Derived from the dependency graph: which slices can run concurrently right now, what waits behind them, and what is held on a human. Change the cap with `/slice-fanout --cap N`.");
w("");
let waveResult;
try {
...
// ---- recently completed ---------------------------------------------------
const windowDays = (graph.meta && graph.meta.completed_window_days) || 14;
...
w("");
w("This file is **generated** from [`roadmap/roadmap.yaml`](roadmap/roadmap.yaml). To change it, edit the YAML and run `/slice-sync` (which reconciles statuses against merged PRs + the tracker, then re-renders). `/slice` reads the YAML directly to orient; `/slice-fanout` reads it to compute waves and launch sessions. Slice **invoke keys are stable** — they are the `/slice` keys and are referenced from `ROADMAP.md`.");
w("");
const seeAlso = [
