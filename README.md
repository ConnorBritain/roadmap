...
- **`roadmap estimate timeline`** rolls those durations up along the **same wave / dependency / concurrency schedule the fanout actually runs** — a wave counts as its slowest slice, waves accumulate — into a `projected_target_date` per PI. That fills Linear's `targetDate` wherever you haven't set an explicit commitment (**an explicit `target_date` always wins**). Unpriced and blocked/gated slices are surfaced, never silently zeroed.
