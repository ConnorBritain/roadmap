...
| `roadmap fan [-t wt\|warp\|tmux\|print\|background] [-c N] [-w N] [--track A] [--harness claude\|codex\|generic] [--emit acp] [--agent <id>] [--lead-claude] [-d] [-o file] [--worker-mode <m>] [--autonomous --yes-spawn-autonomous]` | Launch a wave: a lead pane/tab plus one per slice, each in its own worktree with a synthesized kickoff brief. **Launches by default**; `-d/--dry` or `-o/--out` to preview. `--track <lane>` fans out only the slices in that lane. `--harness` selects the agent CLI (default `claude`; `codex` launches `codex exec`); `--emit acp` writes an [ACP](docs/cross-harness.md) wave manifest instead of launching (for OpenClaw/Hermes/editors), `--agent <id>` names the ACP target. Worker **and** lead sessions take `--permission-mode` from `meta.worker_mode` (falls back to `plan`); `--worker-mode` overrides per run. Terminal defaults per platform (win32 to `wt`, else `tmux`). |
...
**Cross-harness.** `roadmap` isn't Claude-only. The directive's *intent* (mode, count, composition, floor) is harness-neutral; only the *wording* and the *launch command* are dialect. `meta.harness` (`claude` default | `codex` | `generic`) — or `--harness` per run — selects the profile:

- **Directives** reword per harness: `agent-team` renders as "Invoke Agent Teams" on Claude but degrades to "N parallel `codex exec` sessions; the integrator reconciles" on Codex (and a neutral form elsewhere). A non-native mode or unknown harness **warns and degrades/falls back — never errors**.
- **Launch** goes through the profile: `--harness codex` fans out `codex exec` with the mapped `--sandbox`/`--ask-for-approval` flags instead of `claude --permission-mode`. Claude launches are byte-identical to before.
- **Orchestrators** (OpenClaw, Hermes, any [ACP](docs/cross-harness.md) editor) consume `roadmap fan --emit acp` — a JSON wave manifest of `sessions_spawn`-shaped specs (`{ agentId, task, cwd, branch, baseRef, mode }`), the kickoff brief as the `task`. One export covers every ACP consumer.

See [`docs/cross-harness.md`](docs/cross-harness.md) for the two-tier design (launch profiles vs. ACP interop) and the research behind it.
