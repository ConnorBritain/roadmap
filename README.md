...
`target_date` is the one roadmap date that's usually a guess. The roadmap ships the [**agent-time**](https://github.com/ConnorBritain/agent-time) estimation model natively (`lib/estimator-core.mjs`, a port of its `estimator.py`): it prices a task in calibrated *agent-rounds → wall-clock minutes* (never human hours) and self-corrects from logged outcomes. No Python is needed. The calibration history is agent-time's own `history.jsonl` format at the same path (`<repo>/.claude/agent-time/history.jsonl`, or `$AGENT_TIME_DATA`), so an existing history keeps calibrating and agent-time's round-counter hook keeps auto-filling actuals. The roadmap turns those durations into a **projected timeline**:
