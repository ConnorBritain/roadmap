// roadmap — reconcile brain (PURE, engineering executor). Detects slices whose work has merged but
// whose roadmap status still says open, so the SessionStart hook and the PR monitor can nudge the
// agent to reconcile (mark complete + record the PR, then re-render). No IO: the caller supplies the
// merged-PR list (from gh); this matches it against the graph by fanout branch and PR marker.
// Split out of sync-core.mjs when core stopped knowing about branches and PRs (docs/ARCHITECTURE.md).

import { flatten, isDone } from "@connorbritain/roadmap-core/graph.mjs";
import { branchFor } from "./brief.mjs";
import { normalizeExecution, dirClusters } from "@connorbritain/roadmap-core/execution.mjs";
