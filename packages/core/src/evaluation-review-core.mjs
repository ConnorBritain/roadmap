...
import { commentWasEdited } from "./gauntlet-artifact.mjs";
...
export function evaluationReviewStatus(state, pr, { corpusDigest = null } = {}) {
  ...
  if (pr.number !== state.evidence_pr.number || pr.url !== state.evidence_pr.url || pr.baseRef !== state.evidence_pr.base_ref) {
...
export function findEvaluationAttestation(comments, kind, payload, leadActor, prUrl) {
  ...
  return (comments || []).find((comment) => comment.body === body && (comment.author?.login || comment.author) === leadActor
    && typeof comment.url === "string" && comment.url.startsWith(`${prUrl}#`)
    && !commentWasEdited(comment)) || null;
}
