...
import { mutateAuthorization } from "./gauntlet-authority.mjs";
import { prohibitedDataFindings } from "./evaluation-packet.mjs";
import { asGauntletArtifact, commentWasEdited } from "./gauntlet-artifact.mjs";

const OUTCOMES = { finding: ["accepted", "rejected"], regression: ["observed", "cleared"], human_intervention: ["occurred"] };
const hash = (text) => createHash("sha256").update(text).digest("hex");
// A comment locator is backend-defined (a URL, a `<sha>:<path>#<n>`, a document anchor) but always
// `<artifact locator>#<comment id>`: one bounded token, no whitespace, with the anchor. The artifact
// adapter proves it belongs to the artifact; this only bounds its shape.
const LOCATOR_RE = /^\S{1,2000}#\S{1,48}$/;
...
export function recordLeadDecision(state, input, { actor, confirm, now = new Date().toISOString() } = {}) {
  ...
  if (input?.version !== 1 || !/^[A-Za-z0-9_-]{1,120}$/.test(input.id || "")
    ...
    || !/^[a-f0-9]{64}$/.test(input.reference_digest || "") || !LOCATOR_RE.test(input.reference_url || "")
...
// Record a lead decision that references one immutable comment on the artifact at its exact current
// head. `artifact` is a GauntletArtifact (`github` accepted as a legacy alias); `prNumber` is the
// artifact number.
export async function recordDecisionForArtifact({ store, runId, artifact, github, prNumber, number, expectedHead, input, confirm, now }) {
  const adapter = asGauntletArtifact(artifact || github);
  const ref = number != null ? number : prNumber;
  ...
  const pr = await adapter.fetch(ref);
  if (pr.state !== "OPEN" || pr.currentHead !== expectedHead || input?.head !== expectedHead) throw new Error("decision must reference the exact current open artifact head");
  const comment = pr.comments.find((comment) => comment.url === input.reference_url);
  if (!comment || !comment.url.startsWith(`${pr.url}#`) || commentWasEdited(comment)) throw new Error("decision requires an immutable comment on this artifact");
  const record = { ...input, reference_digest: hash(comment.body) };
  const refreshed = await adapter.fetch(ref);
  ...
  if (refreshed.state !== "OPEN" || refreshed.currentHead !== expectedHead || !current || hash(current.body) !== record.reference_digest
    || commentWasEdited(current)) throw new Error("decision reference changed during inspection");
...
export const recordDecisionForPr = recordDecisionForArtifact;
