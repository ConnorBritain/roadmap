...
// The branch convention belongs to the executor (engineering: brief.mjs's branchFor). Core never
// assumes one, so the resolver is injected: branchFor(node, graph) -> branch name.
function requireBranchFor(branchFor) {
  if (typeof branchFor !== "function") throw new Error("journal-core needs a branchFor(node, graph) resolver from the executor");
  return branchFor;
}
...
export function sliceForBranch(graph, branch, branchFor) {
  const resolve = requireBranchFor(branchFor);
  if (!branch) return null;
  const matches = flatten(graph).nodes.filter((n) => resolve(n, graph) === branch);
...
export function autoPostPlan(graph, { branch, commits = [], dirty = "", branchFor } = {}) {
  const node = sliceForBranch(graph, branch, branchFor);
