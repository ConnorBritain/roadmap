...
const SETTABLE = new Set([
  ...
  "shape", "risks", "estimate", "receipts", "outcome", "artifact",
]);
...
// `ctx` is the executor's plan context ({ capacity, annotate }, see plan.mjs); the MCP server passes
// the loaded profile's. Absent → the core default (meta.default_concurrency, no probes).
export function readPlan(graph, args = {}, ctx = {}) {
  return buildPlan(graph, { cap: args.cap, reviewCeiling: args.reviewCeiling, capacity: ctx.capacity, annotate: ctx.annotate });
}
export function readReadyWave(graph, args = {}, ctx = {}) {
  const plan = buildPlan(graph, { cap: args.cap, capacity: ctx.capacity, annotate: ctx.annotate });
