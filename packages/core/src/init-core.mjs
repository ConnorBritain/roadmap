 ...
 */
export function planInit(choices, { existingFiles = new Set(), profiles = {} } = {}) {
  const {
    program = "myproj",
  ...
  if (withLocal) {
    ...
    files.push({
      path: localPath,
      contents: renderLocalConfig({ assistant, profiles }),
...
// `profiles` is the executor's assistant-profile table (engineering: assistant-core's
// BUILTIN_PROFILES); core carries no launch-command knowledge of its own.
export function renderLocalConfig({ assistant = "manual", profiles = {} } = {}) {
  const profile = profiles[assistant] || profiles.manual || {};
