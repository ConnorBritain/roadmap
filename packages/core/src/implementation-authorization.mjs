...
// `artifact` is the GauntletArtifact; an adapter that can host a protected authority journal
// exposes authorityStore() (github-pr: protected Git objects). Tests inject opts.authorityStore.
...
import { asGauntletArtifact } from "./gauntlet-artifact.mjs";

export function implementationAuthorityStore(root, artifact, opts = {}) {
  if (opts.authorityStore) return opts.authorityStore;
  const adapter = asGauntletArtifact(artifact);
  return adapter.supportsProtectedAuthority && typeof adapter.authorityStore === "function"
    ? adapter.authorityStore({ root, execImpl: opts.execImpl }) : null;
}

export async function freezeImplementationAuthority(root, run, policy, artifact, opts = {}) {
  const store = implementationAuthorityStore(root, artifact, opts);
  if (!store) throw new Error("bounded implementation requires a protected-authority-capable artifact adapter");
...
export async function readImplementationAuthority(root, run, artifact, opts = {}) {
  const store = implementationAuthorityStore(root, artifact, opts);
...
export async function reserveImplementationCapacity(root, run, request, artifact, opts = {}) {
  const { store, snapshot } = await readImplementationAuthority(root, run, artifact, opts);
  ...
  if (await asGauntletArtifact(artifact).actor() !== snapshot.state.authorization.lead_actor) throw new Error("bounded launch requires the frozen lead");
